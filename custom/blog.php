...
if (isset($_GET['customid'])) {
  $colname_custom = $_GET['customid'];
}
mysqli_select_db(dbconnect(),$database_rayicecms);
$query_custom = sprintf("SELECT * FROM custom WHERE customid = %s AND status = 'published' ORDER BY customid DESC", GetSQLValueString($colname_custom, "int"));
...
$colname_Comments = "-1";
if (isset($_GET['customid'])) {
  $colname_Comments = $_GET['customid'];
}
mysqli_select_db(dbconnect(),$database_rayicecms);
$query_Comments = sprintf("SELECT * FROM comments WHERE id = %s AND selecttopic = 'custom' AND status = 'published' ORDER BY commentid DESC", GetSQLValueString($colname_Comments, "text"));
...
$getviews = $_GET["customid"];
$views_query = mysqli_query(dbconnect(),"SELECT * FROM custom WHERE customid = $getviews");
$views = mysqli_fetch_assoc($views_query);

if(isset($_GET["customid"]))
{
	$addview = $views['views']+1;
	$viewsupdate_query = mysqli_query(dbconnect(),"UPDATE custom SET views = $addview WHERE customid = $getviews");
}

?>
<?php include($theme_path."".$row_setting['theme']."/topdocs.php"); ?>
<title><?php echo $row_custom['title']; ?></title>
<meta name="keywords" content="<?php echo $row_custom['metakey']; ?>">
<meta name="description" content="<?php echo $row_custom['metadesc']; ?>">
...
<table class="mainbody">
      <tr>
        ...
        <td valign="top"><?php if ($totalRows_custom > 0) { // Show if recordset not empty ?>
              <div class="title"><?php echo $row_custom['title']; ?></div><table class="tables">
                <tr>
                  <td class="texts"><img src="images/post/<?php echo $row_custom['photo']; ?>" width="200" height="200" align="left" style="padding:16px;" /> <?php echo $row_custom['description']; ?></td>
                  <tr><td class="texts"><?php echo $row_custom['users']; ?> / <?php echo $row_custom['dates']; ?> / <a href="category.php?catename=<?php echo $row_custom['catename']; ?>"><?php echo $row_custom['catename']; ?></a></td></tr>
                </tr>
                <tr>
                  <td class="texts">&nbsp;</td>
                </tr>
...
<table class="tables">
                      ...
                      <tr>
                        ...
                        <td><input name="button" type="submit" class="button" id="button" value="Submit">
                            <input name="id" type="hidden" id="id" value="<?php echo $row_custom['customid']; ?>">
