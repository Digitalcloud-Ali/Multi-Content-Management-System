...
$colname_custom = "-1";
if (isset($_GET['title'])) {
  $colname_custom = $_GET['title'];
}
mysqli_select_db(dbconnect(),$database_rayicecms);
$query_custom = sprintf("SELECT * FROM custom WHERE title LIKE %s AND status = 'published' ORDER BY customid DESC", GetSQLValueString("%" . $colname_custom . "%", "text"));
