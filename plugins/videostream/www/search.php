...
$colname_video = "-1";
if (isset($_GET['title'])) {
  $colname_video = $_GET['title'];
}
mysqli_select_db(dbconnect(),$database_rayicecms);
$query_video = sprintf("SELECT * FROM video WHERE title LIKE %s AND status = 'published' ORDER BY videoid DESC", GetSQLValueString("%" . $colname_video . "%", "text"));
...
<?php include($theme_path."".$row_setting['theme']."/header.php"); ?><table class="mainbody">
      <tr>
        ...
        <td valign="top">
		<div class="title">Results</div><?php if ($totalRows_video > 0) { // Show if recordset not empty ?>
            <?php do { ?>
              <table>
                <tr>
                  <td colspan="2"><a href="video.php?videoid=<?php echo $row_video['videoid']; ?>" class="posttitle"><?php echo multicms_h($row_video['title']); ?></a></td>
                </tr>
                <tr>
