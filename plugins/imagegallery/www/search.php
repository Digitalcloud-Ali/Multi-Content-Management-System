...
$colname_image = "-1";
if (isset($_GET['title'])) {
  $colname_image = $_GET['title'];
}
mysqli_select_db(dbconnect(),$database_rayicecms);
$query_image = sprintf("SELECT * FROM image WHERE title LIKE %s AND status = 'published' ORDER BY imageid DESC", GetSQLValueString("%" . $colname_image . "%", "text"));
      ...
      <tr>
		...
		<?php if ($totalRows_image > 0) { // Show if recordset not empty ?>
            <?php do { ?>
              <table>
                <tr>
                  <td colspan="2"><a href="image.php?imageid=<?php echo $row_image['imageid']; ?>" class="posttitle"><?php echo multicms_h($row_image['title']); ?></a></td>
                </tr>
                <tr>
