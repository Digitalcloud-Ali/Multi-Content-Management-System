...
mysqli_select_db(dbconnect(),$database_rayicecms);
$query_portfolio = "SELECT * FROM portfolio WHERE selecttopic = 'portfolio'";
...
<title><?php echo multicms_h($row_portfolio['title']); ?></title>
...
<?php include($theme_path."".$row_setting['theme']."/header.php"); ?><table class="mainbody">
      <tr>
        ...
        <td valign="top"><div class="title">Portfolio</div>
          <?php do { ?>
            <table align="left">
              <tr>
                <td><?php echo multicms_h($row_portfolio['title']); ?></td>
              </tr>
            </table>
          <?php } while ($row_portfolio = mysqli_fetch_assoc($portfolio)); ?></td>
        <?php
