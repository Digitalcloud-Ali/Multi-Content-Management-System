...
class PortfolioPlugin {
    public static function onActivate() {
        ...
        $src = $repoRoot . '/portfolio';
        $dest = __DIR__ . '/www';
        if (!is_dir($src)) return ['success'=>false,'message'=>'Legacy portfolio folder not found'];
        if (is_dir($dest)) return ['success'=>true,'message'=>'Already migrated'];
        if (!self::rcopy($src,$dest)) return ['success'=>false,'message'=>'Failed to copy portfolio files'];
        @file_put_contents($dest.'/.migrated',"migrated_on=".date('c'));
        return ['success'=>true,'message'=>'Portfolio copied to plugins/portfolio/www'];
    }
