...
$colname_adposting = "-1";
if (isset($_GET['title'])) {
  $colname_adposting = $_GET['title'];
}
mysqli_select_db(dbconnect(),$database_rayicecms);
$query_adposting = sprintf("SELECT * FROM adposting WHERE title LIKE %s AND status = 'published' ORDER BY adpostingid DESC", GetSQLValueString("%" . $colname_adposting . "%", "text"));
