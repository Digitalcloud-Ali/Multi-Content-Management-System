...
$colname_blog = "-1";
if (isset($_GET['title'])) {
  $colname_blog = $_GET['title'];
}
mysqli_select_db(dbconnect(),$database_rayicecms);
$query_blog = sprintf("SELECT * FROM blog WHERE title LIKE %s AND status = 'published' ORDER BY blogid DESC", GetSQLValueString("%" . $colname_blog . "%", "text"));
