...
if (isset($_GET['catename'])) {
  $colname_blog = $_GET['catename'];
}
mysqli_select_db(dbconnect(),$database_rayicecms);
$query_blog = sprintf("SELECT * FROM blog WHERE catename = %s AND status = 'published'  ORDER BY blogid DESC", GetSQLValueString($colname_blog, "text"));
...
<title><?php echo multicms_h($row_blog['catename']); ?></title>
<meta name="keywords" content="<?php echo multicms_h($row_blog['catename']); ?>">
<meta name="description" content="<?php echo multicms_h($row_blog['catename']); ?>">
...
<table class="mainbody">
      <tr>
        ...
        <td valign="top">
		<div class="title"><?php echo multicms_h($row_blog['catename']); ?></div>
		...
		<?php if ($totalRows_blog > 0) { // Show if recordset not empty ?>
            <?php do { ?>
              <table class="tables">
                <tr>
                  <td class="orangetitle" style="padding:4px; border:1px;border-style:solid; border-color:#CCCCCC;"><div align="center"><?php echo $row_blog['blogid']; ?></div></td>
                    <td><a href="blog.php?blogid=<?php echo $row_blog['blogid']; ?>" class="posttitle"><?php echo multicms_h($row_blog['title']); ?></a></td>
                  </tr>
                <tr>
