...
if (isset($_GET['catename'])) {
  $colname_tutorials = $_GET['catename'];
}
mysqli_select_db(dbconnect(),$database_rayicecms);
$query_tutorials = sprintf("SELECT * FROM tutorials WHERE catename = %s AND status = 'published' ORDER BY tutorialsid DESC", GetSQLValueString($colname_tutorials, "text"));
...
<title><?php echo multicms_h($row_setting['title']); ?> - <?php echo multicms_h($row_tutorials['catename']); ?></title>
...
<div class="title"><?php echo multicms_h($row_tutorials['catename']); ?></div>
            <?php do { ?>
<table>
       <tr>
