...
mysqli_select_db(dbconnect(),$database_rayicecms);
$query_tutorials = sprintf("SELECT * FROM tutorials WHERE tutorialsid = %s ORDER BY tutorialsid DESC", GetSQLValueString($colname_tutorials, "int"));
...
$getviews = $_GET["tutorialsid"];
$views_query = mysqli_query(dbconnect(),"SELECT * FROM tutorials WHERE tutorialsid = $getviews");
$views = mysqli_fetch_assoc($views_query);

if(isset($_GET["tutorialsid"]))
{
	$addview = $views['views']+1;
	$viewsupdate_query = mysqli_query(dbconnect(),"UPDATE tutorials SET views = $addview WHERE tutorialsid = $getviews");
}
      ...
      <tr>
        ...
        <td valign="top"><?php if ($totalRows_tutorials > 0) { // Show if recordset not empty ?>
                ...
                <table>
                  ...
                  <tr>
                    <td><a href="<?php echo $row_tutorials['tutorialpathurl']; ?>">Visit Tutorial Site</a></td>
                  </tr>
                </table>
