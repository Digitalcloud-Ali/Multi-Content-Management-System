...
$colname_market = "-1";
if (isset($_GET['title'])) {
  $colname_market = $_GET['title'];
}
mysqli_select_db(dbconnect(),$database_rayicecms);
$query_market = sprintf("SELECT * FROM market WHERE title LIKE %s AND status = 'published' ORDER BY marketid DESC", GetSQLValueString("%" . $colname_market . "%", "text"));
...
<?php include($theme_path."".$row_setting['theme']."/header.php"); ?><table class="mainbody">
 ...
 <table class="tables">
        <tr>
          <td valign="top"><div class="title">Search Results</div>
            <?php do { ?>
            <table width="100%">
                    <tr>
                      <td width="1"><a href="market.php?marketid=<?php echo $row_market['marketid']; ?>"><img src="images/market/<?php echo $row_market['imageurl']; ?>" width="40" height="40" border="0"></a></td>
            ...
            <div align="center"><a href="<?php printf("%s?pageNum_market=%d%s", $currentPage, 0, $queryString_market); ?>" class="navigationbuttons">FIRST</a> <a href="<?php printf("%s?pageNum_market=%d%s", $currentPage, max(0, $pageNum_market - 1), $queryString_market); ?>" class="navigationbuttons">PREVIOUS</a> <a href="<?php printf("%s?pageNum_market=%d%s", $currentPage, min($totalPages_market, $pageNum_market + 1), $queryString_market); ?>" class="navigationbuttons">NEXT</a> <a href="<?php printf("%s?pageNum_market=%d%s", $currentPage, $totalPages_market, $queryString_market); ?>" class="navigationbuttons">LAST</a></div></td>
        </tr>
      </table>
