...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "commentupdate")) {
  $updateSQL = sprintf("UPDATE comments SET status=%s WHERE commentid=%s",
...
if ((isset($_GET['commentiddelete'])) && ($_GET['commentiddelete'] != "")) {
  $deleteSQL = sprintf("DELETE FROM comments WHERE commentid=%s",
                       GetSQLValueString($_GET['commentiddelete'], "int"));

  mysqli_select_db(dbconnect(),$database_rayicecms);
  $Result1 = mysqli_query(dbconnect(),$deleteSQL) or die(mysqli_connect_error());

  $deleteGoTo = "comments.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
$colname_commentsoverview = "-1";
if (isset($_SERVER['selecttopic'])) {
  $colname_commentsoverview = $_SERVER['selecttopic'];
}
mysqli_select_db(dbconnect(),$database_rayicecms);
$query_commentsoverview = sprintf("SELECT * FROM comments WHERE selecttopic = %s", GetSQLValueString($colname_commentsoverview, "text"));
...
$queryString_commentsoverview = "";
if (!empty($_SERVER['QUERY_STRING'])) {
  $params = explode("&", $_SERVER['QUERY_STRING']);
  $newParams = array();
  foreach ($params as $param) {
    if (stristr($param, "pageNum_commentsoverview") == false && 
        stristr($param, "totalRows_commentsoverview") == false) {
      array_push($newParams, $param);
    }
  }
  if (count($newParams) != 0) {
    $queryString_commentsoverview = "&" . htmlentities(implode("&", $newParams));
  }
}
$queryString_commentsoverview = sprintf("&totalRows_commentsoverview=%d%s", $totalRows_commentsoverview, $queryString_commentsoverview);
...
<title><?php echo $row_setting['title']; ?> - Comments</title>
...
<?php if($row_setting['selecttopic'] == 'custom')
			...
			if($row_contentlinks['status'] == 'active')
              ...
              <td width="700" height="30" align="center" valign="top"><table width="100%" border="0" cellpadding="4" cellspacing="1" bgcolor="#CCCCCC">
                <tr>
                  <td background="images/leftNavBg.png" class="admintitle">COMMENTS OVERVIEW</td>
                </tr>
              </table>
                <table width="100%" border="0" cellpadding="8" cellspacing="1" bgcolor="#CCCCCC">
                  <tr>
                    <td height="47" align="left" bgcolor="#FFFFFF"><?php if ($totalRows_commentsoverview > 0) { // Show if recordset not empty ?>
  <table width="100%" border="0" cellspacing="4" cellpadding="4">
    <tr class="orangetitle"  style="background-image:url(images/spinner-bg.gif);">
      <td width="30" align="center">ID</td>
      <td width="747">Comment</td>
    </tr>
    <?php do { ?>
      <tr>
        <td height="11" align="center" class="smallbut"><div class="textsmallred"><?php echo $row_commentsoverview['commentid']; ?></div></td>
      ...
      <tr>
        <td height="1" colspan="2" bgcolor="#F8F8F8" class="textsmall"><form id="commentupdate" name="commentupdate" method="POST" action="<?php echo $editFormAction; ?>">
          ...
          <select name="status" class="formmenusimple" id="status">
            ...
            <option value="published" <?php if (!(strcmp("published", $row_commentsoverview['status']))) {echo "selected=\"selected\"";} ?>>published</option>
            </select>
          <input name="button" type="submit" class="smallbut" id="button" value="Go" />
          <input name="commentid" type="hidden" id="commentid" value="<?php echo $row_commentsoverview['commentid']; ?>" />
          <input type="hidden" name="MM_update" value="commentupdate" />
          / <a href="comments.php?commentiddelete=<?php echo $row_commentsoverview['commentid']; ?>" class="textsmallred">Delete ? </a>
        </form></td>
      </tr>
      <?php } while ($row_commentsoverview = mysqli_fetch_assoc($commentsoverview)); ?>
  </table>
  <table width="40" border="0" cellpadding="0" cellspacing="0">
    <tr>
      <td height="4"></td>
  ...
  <table border="0" align="center" cellpadding="0" cellspacing="4">
    <tr>
      <td align="center" class="pagesmove"><a href="<?php printf("%s?pageNum_commentsoverview=%d%s", $currentPage, 0, $queryString_commentsoverview); ?>">&lt;</a></td>
      <td align="center" class="pagesmove"><a href="<?php printf("%s?pageNum_commentsoverview=%d%s", $currentPage, max(0, $pageNum_commentsoverview - 1), $queryString_commentsoverview); ?>">&lt;&lt;</a></td>
      <td align="center" class="pagesmove"><a href="<?php printf("%s?pageNum_commentsoverview=%d%s", $currentPage, min($totalPages_commentsoverview, $pageNum_commentsoverview + 1), $queryString_commentsoverview); ?>">&gt;&gt;</a></td>
      <td align="center" class="pagesmove"><a href="<?php printf("%s?pageNum_commentsoverview=%d%s", $currentPage, $totalPages_commentsoverview, $queryString_commentsoverview); ?>">&gt;</a></td>
    </tr>
  </table>
  <?php } // Show if recordset not empty ?>
                      <?php if ($totalRows_commentsoverview == 0) { // Show if recordset empty ?>
