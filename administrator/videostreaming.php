...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "add")) {
   ...
   // UPLOAD CODE BY SYED RAZA ALI START //
	...
	$max_filesize = 5250000;
  	$upload_path = '../videostream/images/videoimages/';
   ...
   // UPLOAD CODE BY SYED RAZA ALI START //
	...
	$max_filesize1 = 525000000;
  	$upload_path1 = '../videostream/videos/';
  ...
  $insertSQL = sprintf("INSERT INTO video (title, `date`, catename, users, selecttopic) VALUES (%s, %s, %s, %s, %s)",
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "update")) {
   ...
   // UPLOAD CODE BY SYED RAZA ALI START //
   	$allowed_filetypes1 = array('.FLV','.mp3','.mp4','.3gp','.avi');
  	$no_IDD = $row_rsM['videoid'];
	$max_filesize1 = 525000000;
  	$upload_path1 = '../videostream/videos/';
	if($_FILES['videourl']['size'] == 0 || empty($_FILES['videourl']['name']))
		{
   		   $filename1 = $row_rsM['imageurl'];
		}
	else
		{
   		   $filename1 = $no_IDD."".str_replace(' ', '_', $_FILES['videourl']['name']);
  ...
  $updateSQL = sprintf("UPDATE video SET title=%s, `description`=%s, imageurl=%s, videourl=%s, embedcode=%s, `date`=%s, videotype=%s, catename=%s, status=%s, rating=%s, views=%s, `position`=%s, users=%s WHERE videoid=%s",
                       GetSQLValueString($_POST['title'], "text"),
...
mysqli_select_db(dbconnect(),$database_rayicecms);
$query_blogoverview = "SELECT * FROM video ORDER BY videoid DESC";
...
if (isset($_GET['videoid'])) {
  $colname_blogupdate = $_GET['videoid'];
}
mysqli_select_db(dbconnect(),$database_rayicecms);
$query_blogupdate = sprintf("SELECT * FROM video WHERE videoid = %s", GetSQLValueString($colname_blogupdate, "int"));
...
mysqli_select_db(dbconnect(),$database_rayicecms);
$query_categorylist = "SELECT * FROM categories WHERE selecttopic = 'videostream'";
...
<title><?php echo $row_setting['title']; ?> - Video Streaming</title>
...
<?php if($row_setting['selecttopic'] == 'custom')
			...
			if($row_contentlinks['status'] == 'active')
              ...
              <td width="700" height="30" align="center" valign="top"><?php if ($totalRows_blogupdate == 0) { // Show if recordset empty ?>
                  <table width="100%" border="0" cellpadding="4" cellspacing="1" bgcolor="#CCCCCC">
                    <tr>
                      <td background="images/leftNavBg.png" class="admintitle">VIDEO OVERVIEW</td>
                    </tr>
                  </table>
  ...
  <table width="100%" border="0" cellspacing="2" cellpadding="4">
    ...
    <?php do { ?>
      <tr>
        <td height="11" align="center" class="smallbut"><div class="textsmallred"><?php echo $row_blogoverview['videoid']; ?></div></td>
        <td bgcolor="#FBFEFF" class="borderorangetable"><div class="texts"><?php echo $row_blogoverview['title']; ?></div></td>
        <td width="1" align="center" bgcolor="#FBFEFF" class="borderorangetable"><a href="videostreaming.php?videoid=<?php echo $row_blogoverview['videoid']; ?>" class="textsmallgreen">
          <div>Edit</div>
        </a></td>
        <td width="1" align="center" bgcolor="#FBFEFF" class="borderorangetable"><a href="videostreaming.php?delete=<?php echo $row_blogoverview['videoid']; ?>" class="textsmallred" onClick="javascript:return confirm('Please Confirm Before Delete ?')">
          <div>Delete</div>
        </a></td>
      </tr>
  ...
  <?php } // Show if recordset not empty ?>
                        <?php if ($totalRows_blogoverview == 0) { // Show if recordset empty ?>
                          <table width="100%" border="0" cellspacing="2" cellpadding="4">
                            <tr class="orangetitle">
                              <td width="32" align="center">NO VIDEO FOUNDED</td>
                            </tr>
                          </table>
                  ...
                  <?php if ($totalRows_blogupdate == 0) { // Show if recordset empty ?>
                    <table width="100%" border="0" cellpadding="4" cellspacing="1" bgcolor="#CCCCCC">
                      <tr>
                        <td background="images/leftNavBg.png" class="admintitle">ADD VIDEO</td>
                      </tr>
                    </table>
                    <table width="100%" border="0" cellpadding="8" cellspacing="1" bgcolor="#CCCCCC">
                      <tr>
                        <td height="47" align="left" bgcolor="#FFFFFF"><form action="<?php echo $editFormAction; ?>" method="POST" enctype="multipart/form-data" name="add" id="add">
                          <table width="100%" height="308" border="0" cellpadding="0" cellspacing="4">
                            <tr>
...
?>
                          ...
                          <input name="selecttopic" type="hidden" id="selecttopic" value="videostream" />
  ...
  <table width="100%" border="0" cellpadding="4" cellspacing="1" bgcolor="#ECCF5E">
    <tr>
      <td align="center" bgcolor="#FAF1D1" class="texts"><a href="videostreaming.php" class="textsmallred">Back To Video Streaming</a></td>
    </tr>
  </table>
  <table width="40" border="0" cellpadding="0" cellspacing="0">
    <tr>
  ...
  <table width="100%" border="0" cellpadding="4" cellspacing="1" bgcolor="#CCCCCC">
    <tr>
      <td background="images/leftNavBg.png" class="admintitle">UPDATE VIDEO</td>
      </tr>
  </table>
                    <table width="100%" border="0" cellpadding="8" cellspacing="1" bgcolor="#CCCCCC">
                      <tr>
                        <td height="497" align="left" bgcolor="#FFFFFF"><form action="<?php echo $editFormAction; ?>" method="POST" enctype="multipart/form-data" name="update" id="update">
                          <table width="100%" height="308" border="0" cellpadding="0" cellspacing="4">
                            <tr>
                              <td width="104" height="22" class="texts"><strong>TITLE:*</strong></td>
...
?>
                            ...
                            <tr>
                              <td height="24" class="texts"><strong>IMAGE PATH:*</strong></td>
                              <td><input name="imageurl" type="file" class="formmenusimple" id="imageurl" /><?php echo $row_blogupdate['imageurl']; ?></td>
                            </tr>
                            <tr>
                              <td height="24" class="texts"><strong>VIDEO TYPE:*</strong></td>
                              <td><select name="videotype" class="formmenu" id="videotype">
                                <option value="video" <?php if (!(strcmp("video", $row_blogupdate['videotype']))) {echo "selected=\"selected\"";} ?>>video</option>
                                <option value="embed" <?php if (!(strcmp("embed", $row_blogupdate['videotype']))) {echo "selected=\"selected\"";} ?>>embed</option>
                              </select></td>
                            </tr>
                            <tr>
                              <td height="24" class="texts"><strong>VIDEO PATH:</strong></td>
                              <td><input name="videourl" type="file" class="formmenusimple" id="videourl" /><?php echo $row_blogupdate['videourl']; ?></td>
                            </tr>
                            <tr>
                              <td height="23" valign="top" class="texts"><strong>EMBED CODE:</strong></td>
                              <td><textarea name="embedcode" cols="55" rows="5" class="formtextarea" id="embedcode"><?php echo $row_blogupdate['embedcode']; ?></textarea></td>
                            </tr>
                            <tr>
                              <td height="24" class="texts"><strong>POSITION:</strong></td>
                              <td>
                                <select name="position" class="formmenu" id="position4">
                            ...
                            <tr>
                              <td height="24" class="texts"><strong>STATUS:</strong></td>
                              <td><select name="status" class="formmenu" id="status4">
                          ...
                          <input name="videoid" type="hidden" id="videoid" value="<?php echo $row_blogupdate['videoid']; ?>" />
