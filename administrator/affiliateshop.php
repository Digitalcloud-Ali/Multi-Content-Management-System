...
if (isset($_GET['productpublisherid'])) {
  $colname_rsM = $_GET['productpublisherid'];
}
mysqli_select_db(dbconnect(),$database_rayicecms);
$query_rsM = sprintf("SELECT * FROM productpublisher WHERE productpublisherid = %s", GetSQLValueString($colname_rsM, "int"));
...
mysqli_select_db(dbconnect(),$database_rayicecms);
$query_rsT = "SELECT * FROM productpublisher ORDER BY productpublisherid DESC";
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "add")) {
   ...
   // UPLOAD CODE BY SYED RAZA ALI START //
  	...
  	$no_ID = $row_rsT['productpublisherid'];
	$max_filesize = 5250000;
  	$upload_path = '../productpublisher/images/productpublisher/';
  ...
  $insertSQL = sprintf("INSERT INTO productpublisher (title, `description`, imageurl, productpublisherurl, owner, `date`, catename, status, rating, views, `position`, editorpick, users) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                       GetSQLValueString($_POST['title'], "text"),
                       GetSQLValueString($_POST['description'], "text"),
                       GetSQLValueString($filename, "text"),
                       GetSQLValueString($_POST['productpublisherurl'], "text"),
  ...
  $insertSQL = sprintf("INSERT INTO reports (title, `date`, catename, users, selecttopic) VALUES (%s, %s, %s, %s, %s)",
                       GetSQLValueString($_POST['title'], "text"),
                       GetSQLValueString($_POST['date'], "text"),
                       GetSQLValueString($_POST['catename'], "text"),
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "update")) {
  ...
  // UPLOAD CODE BY SYED RAZA ALI START //
   	$allowed_filetypes = array('.jpg','.gif','.bmp','.png');
  	$no_ID = $row_rsM['productpublisherid'];
	$max_filesize = 5250000;
  	$upload_path = '../productpublisher/images/productpublisher/';
	if($_FILES['imageurl']['size'] == 0 || empty($_FILES['imageurl']['name']))
		{
  ...
  $updateSQL = sprintf("UPDATE productpublisher SET title=%s, `description`=%s, imageurl=%s, productpublisherurl=%s, `date`=%s, catename=%s, status=%s, rating=%s, views=%s, `position`=%s, editorpick=%s, users=%s WHERE productpublisherid=%s",
                       GetSQLValueString($_POST['title'], "text"),
                       GetSQLValueString($_POST['description'], "text"),
                       ...
                       GetSQLValueString($_POST['editorpick'], "text"),
                       GetSQLValueString($_POST['users'], "text"),
                       GetSQLValueString($_POST['productpublisherid'], "int"));

  mysqli_select_db(dbconnect(),$database_rayicecms);
  $Result1 = mysqli_query(dbconnect(),$updateSQL) or die(mysqli_connect_error());

  $updateGoTo = "affiliateshop.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
if ((isset($_GET['delete'])) && ($_GET['delete'] != "")) {
  $deleteSQL = sprintf("DELETE FROM productpublisher WHERE productpublisherid=%s",
                       GetSQLValueString($_GET['delete'], "int"));

  mysqli_select_db(dbconnect(),$database_rayicecms);
  $Result1 = mysqli_query(dbconnect(),$deleteSQL) or die(mysqli_connect_error());

  $deleteGoTo = "affiliateshop.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
mysqli_select_db(dbconnect(),$database_rayicecms);
$query_blogoverview = "SELECT * FROM productpublisher ORDER BY productpublisherid DESC";
...
if (isset($_GET['productpublisherid'])) {
  $colname_blogupdate = $_GET['productpublisherid'];
}
mysqli_select_db(dbconnect(),$database_rayicecms);
$query_blogupdate = sprintf("SELECT * FROM productpublisher WHERE productpublisherid = %s", GetSQLValueString($colname_blogupdate, "int"));
...
mysqli_select_db(dbconnect(),$database_rayicecms);
$query_categorylist = "SELECT * FROM categories WHERE selecttopic = 'productpublisher'";
...
<title><?php echo $row_setting['title']; ?> - Affiliate Shop</title>
...
<?php if($row_setting['selecttopic'] == 'custom')
			...
			if($row_contentlinks['status'] == 'active')
              ...
              <td width="700" height="30" align="center" valign="top"><?php if ($totalRows_blogupdate == 0) { // Show if recordset empty ?>
                  <table width="100%" border="0" cellpadding="4" cellspacing="1" bgcolor="#CCCCCC">
                    <tr>
                      <td background="images/leftNavBg.png" class="admintitle">AFFILIATE OVERVIEW</td>
                    </tr>
                  </table>
                    ...
                    <tr>
                      <td height="47" align="left" bgcolor="#FFFFFF"><?php if ($totalRows_blogoverview > 0) { // Show if recordset not empty ?>
                          <table width="100%" border="0" cellspacing="2" cellpadding="4">
                            ...
                            <?php do { ?>
                              <tr>
                                <td height="11" align="center" class="smallbut"><div class="textsmallred"><?php echo $row_blogoverview['productpublisherid']; ?></div></td>
                                <td bgcolor="#FBFEFF" class="borderorangetable"><div class="texts"><?php echo $row_blogoverview['title']; ?></div></td>
                                <td width="1" align="center" bgcolor="#FBFEFF" class="borderorangetable"><a href="affiliateshop.php?productpublisherid=<?php echo $row_blogoverview['productpublisherid']; ?>" class="textsmallgreen">
                                  <div>Edit</div>
                                </a></td>
                                <td width="1" align="center" bgcolor="#FBFEFF" class="borderorangetable"><a href="affiliateshop.php?delete=<?php echo $row_blogoverview['productpublisherid']; ?>" class="textsmallred" onClick="javascript:return confirm('Please Confirm Before Delete ?')">
                                  <div>Delete</div>
                                </a></td>
                              </tr>
  ...
  <table width="100%" border="0" cellspacing="2" cellpadding="4">
    <tr class="orangetitle">
      <td width="32" align="center">NO PRODUCT FOUNDED</td>
      </tr>
  </table>
  <?php } // Show if recordset empty ?></td>
                  ...
                  <?php if ($totalRows_blogupdate == 0) { // Show if recordset empty ?>
                    <table width="100%" border="0" cellpadding="4" cellspacing="1" bgcolor="#CCCCCC">
                      <tr>
                        <td background="images/leftNavBg.png" class="admintitle">ADD AFFILIATE PRODUCT</td>
                      </tr>
                    </table>
                    <table width="100%" border="0" cellpadding="8" cellspacing="1" bgcolor="#CCCCCC">
                      <tr>
...
?>
                            ...
                            <tr>
                              <td height="24" class="texts"><strong>PRODUCT URL:</strong></td>
                              <td><input name="productpublisherurl" type="text" class="form" id="productpublisherurl" /></td>
                            </tr>
                            <tr>
                              <td height="24" class="texts"><strong>EDITIORS PICK:</strong></td>
                              <td><select name="editorpick" class="formmenu" id="editorpick">
                          ...
                          <input name="selecttopic" type="hidden" id="selecttopic" value="productpublisher" />
                          <input name="users" type="hidden" id="users" value="<?php echo $row_setting['datauser']; ?>" />
                          <input name="owner" type="hidden" id="owner" value="<?php echo $row_setting['datauser']; ?>" />
<input type="hidden" name="MM_insert" value="add" />
  ...
  frmvalidator.addValidation("productpublisherurl","req","Please Provide Refer Link");
  ...
  <table width="100%" border="0" cellpadding="4" cellspacing="1" bgcolor="#ECCF5E">
    <tr>
      <td align="center" bgcolor="#FAF1D1" class="texts"><a href="affiliateshop.php" class="textsmallred">Back To Affiliate Shop</a></td>
    </tr>
  </table>
  <table width="40" border="0" cellpadding="0" cellspacing="0">
    <tr>
  ...
  <table width="100%" border="0" cellpadding="4" cellspacing="1" bgcolor="#CCCCCC">
    <tr>
      <td background="images/leftNavBg.png" class="admintitle">UPDATE AFFILIATE PRODUCT</td>
      </tr>
  </table>
                    <table width="100%" border="0" cellpadding="8" cellspacing="1" bgcolor="#CCCCCC">
                      <tr>
                        <td height="47" align="left" bgcolor="#FFFFFF"><form action="<?php echo $editFormAction; ?>" method="POST" enctype="multipart/form-data" name="update" id="update">
                          <table width="100%" height="281" border="0" cellpadding="0" cellspacing="4">
                            ...
                            <tr>
                              <td height="24" class="texts"><strong>DATE:</strong></td>
                              <td><input name="date" type="text" class="formmenusimple" id="date" value="<?php echo $row_blogupdate['date']; ?>" />
                                <span class="textsmall">FORMAT : DD-MM-YYYY</span></td>
                              </tr>
                            <tr>
                              <td height="24" class="texts"><strong>CATEGORIES:</strong></td>
...
do {  
?>
                                <option value="<?php echo $row_categorylist['catename']?>"<?php if (!(strcmp($row_categorylist['catename'], $row_blogupdate['catename']))) {echo "selected=\"selected\"";} ?>><?php echo $row_categorylist['catename']?></option>
                                <?php
} while ($row_categorylist = mysqli_fetch_assoc($categorylist));
  $rows = mysqli_num_rows($categorylist);
  if($rows > 0) {
...
?>
                            ...
                            <tr>
                              <td height="24" class="texts"><strong>IMAGE:</strong></td>
                              <td><input name="imageurl" type="file" class="formmenusimple" id="imageurl" /><?php echo $row_blogupdate['imageurl']; ?></td>
                              </tr>
                            <tr>
                              <td height="24" class="texts"><strong>PRODUCT URL:</strong></td>
                              <td><input name="productpublisherurl" type="text" class="form" id="productpublisherurl" value="<?php echo $row_blogupdate['productpublisherurl']; ?>" /></td>
                            </tr>
                            <tr>
                              <td height="24" class="texts"><strong>EDITIORS PICK:</strong></td>
                              <td><select name="editorpick" class="formmenu" id="editorpick">
                                <option value="yes" <?php if (!(strcmp("yes", $row_blogupdate['editorpick']))) {echo "selected=\"selected\"";} ?>>yes</option>
                                <option value="no" <?php if (!(strcmp("no", $row_blogupdate['editorpick']))) {echo "selected=\"selected\"";} ?>>no</option>
                                </select></td>
                            </tr>
                            <tr>
                              <td height="24" class="texts"><strong>POSITION:</strong></td>
                          ...
                          <input name="productpublisherid" type="hidden" id="productpublisherid" value="<?php echo $row_blogupdate['productpublisherid']; ?>" />
