...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "add")) {
   ...
   // UPLOAD CODE BY SYED RAZA ALI START //
  	...
  	$no_ID = $row_rsT['customid'];
	$max_filesize = 5250000;
  	$upload_path = '../custom/images/post/';
  ...
  $insertSQL = sprintf("INSERT INTO custom (title, `description`, photo, dates, metadesc, metakey, catename, status, `position`, users, rating, views) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
...
$maxRows_customoverview = 20;
$pageNum_customoverview = 0;
if (isset($_GET['pageNum_customoverview'])) {
  $pageNum_customoverview = $_GET['pageNum_customoverview'];
}
$startRow_customoverview = $pageNum_customoverview * $maxRows_customoverview;

mysqli_select_db(dbconnect(),$database_rayicecms);
$query_customoverview = "SELECT * FROM custom ORDER BY customid DESC";
$query_limit_customoverview = sprintf("%s LIMIT %d, %d", $query_customoverview, $startRow_customoverview, $maxRows_customoverview);
$customoverview = mysqli_query(dbconnect(),$query_limit_customoverview) or die(mysqli_connect_error());
$row_customoverview = mysqli_fetch_assoc($customoverview);

if (isset($_GET['totalRows_customoverview'])) {
  $totalRows_customoverview = $_GET['totalRows_customoverview'];
} else {
  $all_customoverview = mysqli_query(dbconnect(),$query_customoverview);
  $totalRows_customoverview = mysqli_num_rows($all_customoverview);
}
$totalPages_customoverview = ceil($totalRows_customoverview/$maxRows_customoverview)-1;

$colname_customupdate = "-1";
if (isset($_GET['customid'])) {
  $colname_customupdate = $_GET['customid'];
}
mysqli_select_db(dbconnect(),$database_rayicecms);
$query_customupdate = sprintf("SELECT * FROM custom WHERE customid = %s", GetSQLValueString($colname_customupdate, "int"));
$customupdate = mysqli_query(dbconnect(),$query_customupdate) or die(mysqli_connect_error());
$row_customupdate = mysqli_fetch_assoc($customupdate);
$totalRows_customupdate = mysqli_num_rows($customupdate);

mysqli_select_db(dbconnect(),$database_rayicecms);
$query_categorylist = "SELECT * FROM categories WHERE selecttopic = 'custom'";
...
$queryString_customoverview = "";
if (!empty($_SERVER['QUERY_STRING'])) {
  $params = explode("&", $_SERVER['QUERY_STRING']);
  $newParams = array();
  foreach ($params as $param) {
    if (stristr($param, "pageNum_customoverview") == false && 
        stristr($param, "totalRows_customoverview") == false) {
      array_push($newParams, $param);
    }
  }
  if (count($newParams) != 0) {
    $queryString_customoverview = "&" . htmlentities(implode("&", $newParams));
  }
}
$queryString_customoverview = sprintf("&totalRows_customoverview=%d%s", $totalRows_customoverview, $queryString_customoverview);
...
<title><?php echo $row_setting['title']; ?> - custom</title>
...
<?php if($row_setting['selecttopic'] == 'custom')
              ...
              <table width="100%" border="0" cellpadding="0" cellspacing="0" class="leftmenu">
                <tr>
                  <td height="35"><a href="custom.php">
                    <div style="line-height:37px;"><img src="images/addressBook.png" alt="Portal custom" width="14" height="14" border="0" style="padding-left:9px;padding-right:7px;" /> Custom
                    </div></a></td>
                  </tr>
                </table>
                <table width="40" border="0" cellpadding="0" cellspacing="0">
			...
			if($row_contentlinks['status'] == 'active')
			  ...
			  ?></td>
              <td width="13">&nbsp;</td>
              <td width="700" height="30" align="center" valign="top"><?php if ($totalRows_customupdate == 0) { // Show if recordset empty ?>
                  <table width="100%" border="0" cellpadding="4" cellspacing="1" bgcolor="#CCCCCC">
                    <tr>
                      <td background="images/leftNavBg.png" class="admintitle">NEWS / custom OVERVIEW</td>
                    </tr>
                  </table>
                  <table width="100%" border="0" cellpadding="8" cellspacing="1" bgcolor="#CCCCCC">
                    <tr>
                      <td height="47" align="left" bgcolor="#FFFFFF"><?php if ($totalRows_customoverview > 0) { // Show if recordset not empty ?>
  <table width="100%" border="0" cellspacing="2" cellpadding="4">
    ...
    <?php do { ?>
      <tr>
        <td height="11" align="center" class="smallbut"><div class="textsmallred"><?php echo $row_customoverview['customid']; ?></div></td>
        <td bgcolor="#FBFEFF" class="borderorangetable"><div class="texts"><?php echo $row_customoverview['title']; ?></div></td>
        <td width="1" align="center" bgcolor="#FBFEFF" class="borderorangetable"><a href="custom.php?customid=<?php echo $row_customoverview['customid']; ?>" class="textsmallgreen">
          <div>Edit</div>
        </a></td>
        <td width="1" align="center" bgcolor="#FBFEFF" class="borderorangetable"><a href="custom.php?delete=<?php echo $row_customoverview['customid']; ?>" class="textsmallred" onClick="javascript:return confirm('Please Confirm Before Delete ?')">
          <div>Delete</div>
        </a></td>
      </tr>
      <?php } while ($row_customoverview = mysqli_fetch_assoc($customoverview)); ?>
  </table>
  <table width="40" border="0" cellpadding="0" cellspacing="0">
    <tr>
      <td height="4"></td>
  ...
  <table border="0" align="center" cellpadding="0" cellspacing="4">
    <tr>
      <td align="center" class="pagesmove"><a href="<?php printf("%s?pageNum_customoverview=%d%s", $currentPage, 0, $queryString_customoverview); ?>">&lt;</a></td>
      <td align="center" class="pagesmove"><a href="<?php printf("%s?pageNum_customoverview=%d%s", $currentPage, max(0, $pageNum_customoverview - 1), $queryString_customoverview); ?>">&lt;&lt;</a></td>
      <td align="center" class="pagesmove"><a href="<?php printf("%s?pageNum_customoverview=%d%s", $currentPage, min($totalPages_customoverview, $pageNum_customoverview + 1), $queryString_customoverview); ?>">&gt;&gt;</a></td>
      <td align="center" class="pagesmove"><a href="<?php printf("%s?pageNum_customoverview=%d%s", $currentPage, $totalPages_customoverview, $queryString_customoverview); ?>">&gt;</a></td>
    </tr>
  </table>
  <?php } // Show if recordset not empty ?>
                        <?php if ($totalRows_customoverview == 0) { // Show if recordset empty ?>
                          <table width="100%" border="0" cellspacing="2" cellpadding="4">
                            <tr class="orangetitle">
                              <td width="32" align="center">NO POST FOUNDED</td>
                            </tr>
                          </table>
                  ...
                  <?php if ($totalRows_customupdate == 0) { // Show if recordset empty ?>
                    <table width="100%" border="0" cellpadding="4" cellspacing="1" bgcolor="#CCCCCC">
                      <tr>
                        <td background="images/leftNavBg.png" class="admintitle">ADD POST</td>
                      </tr>
                    </table>
                    <table width="100%" border="0" cellpadding="8" cellspacing="1" bgcolor="#CCCCCC">
                      <tr>
                        <td height="47" align="left" bgcolor="#FFFFFF"><form action="<?php echo $editFormAction; ?>" method="POST" enctype="multipart/form-data" name="add" id="add">
                          <table width="100%" height="281" border="0" cellpadding="0" cellspacing="4">
                            <tr>
...
do {  
?>
                                <option value="<?php echo $row_categorylist['catename']?>"><?php echo $row_categorylist['catename']?></option>
                                <?php
} while ($row_categorylist = mysqli_fetch_assoc($categorylist));
  $rows = mysqli_num_rows($categorylist);
  if($rows > 0) {
...
?>
                            ...
                            <tr>
                              <td height="24">&nbsp;</td>
                              <td><input name="button2" type="submit" class="button" id="button2" value="Add Post" /></td>
                            </tr>
                          </table>
  ...
  frmvalidator.addValidation("metakey","maxlen=250","Max length for Meta Keyword is 250");
                    ...
                    <?php } // Show if recordset empty ?>
                  <?php if ($totalRows_customupdate > 0) { // Show if recordset not empty ?>
  <table width="100%" border="0" cellpadding="4" cellspacing="1" bgcolor="#ECCF5E">
    <tr>
      <td align="center" bgcolor="#FAF1D1" class="texts"><a href="custom.php" class="textsmallred">Back To custom</a></td>
    </tr>
  </table>
  <table width="40" border="0" cellpadding="0" cellspacing="0">
    <tr>
  ...
  <table width="100%" border="0" cellpadding="4" cellspacing="1" bgcolor="#CCCCCC">
    <tr>
      <td background="images/leftNavBg.png" class="admintitle">UPDATE POST</td>
      </tr>
  </table>
                    <table width="100%" border="0" cellpadding="8" cellspacing="1" bgcolor="#CCCCCC">
                      <tr>
                        <td height="47" align="left" bgcolor="#FFFFFF"><form action="<?php echo $editFormAction; ?>" method="POST" enctype="multipart/form-data" name="update" id="update">
                          <table width="100%" height="281" border="0" cellpadding="0" cellspacing="4">
                            <tr>
                              <td width="101" height="22" class="texts"><strong>TITLE:*</strong></td>
                              <td width="728"><input name="title" type="text" class="form" id="title" value="<?php echo $row_customupdate['title']; ?>" /></td>
                              </tr>
                            <tr>
                              <td height="23" valign="top" class="texts"><strong>DESCRIPTION:*</strong></td>
                              <td><textarea name="description" cols="55" rows="5" class="wysiwyg" id="description"><?php echo $row_customupdate['description']; ?></textarea></td>
                              </tr>
                            <tr>
                              <td height="24" class="texts"><strong>DATE:*</strong></td>
                              <td><input name="dates" type="text" class="formmenusimple" id="dates" value="<?php echo $row_customupdate['dates']; ?>" />
                                <span class="textsmall">FORMAT : DD-MM-YYYY</span></td>
                              </tr>
                            <tr>
                              <td height="24" class="texts"><strong>PHOTO:</strong></td>
                              <td><input name="photo" type="file" class="formmenusimple" id="photo" /></td>
                            </tr>
                            <tr>
                              <td height="24" class="texts"><strong>CATEGORIES:*</strong></td>
...
do {  
?>
                                <option value="<?php echo $row_categorylist['catename']?>"<?php if (!(strcmp($row_categorylist['catename'], $row_customupdate['catename']))) {echo "selected=\"selected\"";} ?>><?php echo $row_categorylist['catename']?></option>
                                <?php
} while ($row_categorylist = mysqli_fetch_assoc($categorylist));
  $rows = mysqli_num_rows($categorylist);
  if($rows > 0) {
...
?>
                            ...
                            <tr>
                              <td height="24" class="texts"><strong>MEAT DESC:*</strong></td>
                              <td><input name="metadesc" type="text" class="form" id="metadesc" value="<?php echo $row_customupdate['metadesc']; ?>" /></td>
                              </tr>
                            <tr>
                              <td height="24" class="texts"><strong>META KEY:*</strong></td>
                              <td><input name="metakey" type="text" class="form" id="metakey" value="<?php echo $row_customupdate['metakey']; ?>" /></td>
                              </tr>
                            <tr>
                              <td height="24" class="texts"><strong>POSITION:</strong></td>
                              <td>
                                <select name="position" class="formmenu" id="position">
                                  <option value="normal" <?php if (!(strcmp("normal", $row_customupdate['position']))) {echo "selected=\"selected\"";} ?>>normal</option>
                                  <option value="featured" <?php if (!(strcmp("featured", $row_customupdate['position']))) {echo "selected=\"selected\"";} ?>>featured</option>
                                </select></td>
                              </tr>
                            <tr>
                              <td height="24" class="texts"><strong>STATUS:</strong></td>
                              <td><select name="status" class="formmenu" id="status">
                                <option value="pending" <?php if (!(strcmp("pending", $row_customupdate['status']))) {echo "selected=\"selected\"";} ?>>pending</option>
                                <option value="published" <?php if (!(strcmp("published", $row_customupdate['status']))) {echo "selected=\"selected\"";} ?>>published</option>
                                </select></td>
                              </tr>
                            <tr>
                              <td height="24">&nbsp;</td>
                          ...
                          <input name="rating" type="hidden" id="rating" value="<?php echo $row_customupdate['rating']; ?>" />
                          <input name="views" type="hidden" id="views" value="<?php echo $row_customupdate['views']; ?>" />
                          <input name="users" type="hidden" id="users" value="<?php echo $row_customupdate['users']; ?>" />
                          <input name="customid" type="hidden" id="customid" value="<?php echo $row_customupdate['customid']; ?>" />
...
mysqli_free_result($customoverview);

mysqli_free_result($customupdate);
