...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "cateupdate")) {
  $updateSQL = sprintf("UPDATE categories SET catename=%s WHERE cateid=%s",
                       GetSQLValueString($_POST['catename'], "text"),
                       GetSQLValueString($_POST['cateid'], "int"));
...
if ((isset($_GET['cateiddelete'])) && ($_GET['cateiddelete'] != "")) {
  $deleteSQL = sprintf("DELETE FROM categories WHERE cateid=%s",
                       GetSQLValueString($_GET['cateiddelete'], "int"));
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "cateinsert")) {
  $insertSQL = sprintf("INSERT INTO categories (cateid, catename, selecttopic) VALUES (%s, %s, %s)",
                       GetSQLValueString($_POST['cateid'], "int"),
                       GetSQLValueString($_POST['catename'], "text"),
...
$colname_catesoverview = "-1";
if (isset($_SERVER['selecttopic'])) {
  $colname_catesoverview = $_SERVER['selecttopic'];
}
mysqli_select_db(dbconnect(),$database_rayicecms);
$query_catesoverview = sprintf("SELECT * FROM categories WHERE selecttopic = %s ORDER BY cateid DESC", GetSQLValueString($colname_catesoverview, "text"));
...
$colname_catesupdate = "-1";
if (isset($_GET['cateid'])) {
  $colname_catesupdate = $_GET['cateid'];
}
mysqli_select_db(dbconnect(),$database_rayicecms);
$query_catesupdate = sprintf("SELECT * FROM categories WHERE cateid = %s", GetSQLValueString($colname_catesupdate, "int"));
$catesupdate = mysqli_query(dbconnect(),$query_catesupdate) or die(mysqli_connect_error());
$row_catesupdate = mysqli_fetch_assoc($catesupdate);
$totalRows_catesupdate = mysqli_num_rows($catesupdate);
...
$queryString_catesoverview = "";
if (!empty($_SERVER['QUERY_STRING'])) {
  $params = explode("&", $_SERVER['QUERY_STRING']);
  $newParams = array();
  foreach ($params as $param) {
    if (stristr($param, "pageNum_catesoverview") == false && 
        stristr($param, "totalRows_catesoverview") == false) {
      array_push($newParams, $param);
    }
  }
  if (count($newParams) != 0) {
    $queryString_catesoverview = "&" . htmlentities(implode("&", $newParams));
  }
}
$queryString_catesoverview = sprintf("&totalRows_catesoverview=%d%s", $totalRows_catesoverview, $queryString_catesoverview);
...
<title><?php echo $row_setting['title']; ?> - Categories</title>
...
<?php if($row_setting['selecttopic'] == 'custom')
			...
			if($row_contentlinks['status'] == 'active')
			  ...
			  ?></td>
              <td width="13">&nbsp;</td>
              <td width="700" height="30" align="center" valign="top"><?php if ($totalRows_catesupdate == 0) { // Show if recordset empty ?>
                  <table width="100%" border="0" cellpadding="4" cellspacing="1" bgcolor="#CCCCCC">
                    <tr>
                      <td background="images/leftNavBg.png" class="admintitle">categories</td>
                    </tr>
                  </table>
  ...
  <table width="100%" border="0" cellspacing="2" cellpadding="4">
    ...
    <?php do { ?>
      <tr>
        ...
        <td bgcolor="#FBFEFF" class="borderorangetable"><div class="texts"><?php echo $row_catesoverview['catename']; ?></div></td>
        <td width="1" align="center" bgcolor="#FBFEFF" class="borderorangetable"><a href="categories.php?cateid=<?php echo $row_catesoverview['cateid']; ?>" class="textsmallgreen">
          <div>Edit</div>
        </a></td>
        <td width="1" align="center" bgcolor="#FBFEFF" class="borderorangetable"><a href="categories.php?cateiddelete=<?php echo $row_catesoverview['cateid']; ?>" class="textsmallred" onClick="javascript:return confirm('Please Confirm Before Delete ? Warning!:All Content Within this category will be deleted as well')">
          <div>Delete</div>
        </a></td>
      </tr>
  ...
  <table border="0" align="center" cellpadding="0" cellspacing="4">
    <tr>
      <td align="center" class="pagesmove"><a href="<?php printf("%s?pageNum_catesoverview=%d%s", $currentPage, 0, $queryString_catesoverview); ?>">&lt;</a></td>
      <td align="center" class="pagesmove"><a href="<?php printf("%s?pageNum_catesoverview=%d%s", $currentPage, max(0, $pageNum_catesoverview - 1), $queryString_catesoverview); ?>">&lt;&lt;</a></td>
      <td align="center" class="pagesmove"><a href="<?php printf("%s?pageNum_catesoverview=%d%s", $currentPage, min($totalPages_catesoverview, $pageNum_catesoverview + 1), $queryString_catesoverview); ?>">&gt;&gt;</a></td>
      <td align="center" class="pagesmove"><a href="<?php printf("%s?pageNum_catesoverview=%d%s", $currentPage, $totalPages_catesoverview, $queryString_catesoverview); ?>">&gt;</a></td>
    </tr>
  </table>
...
{
?>
                  <?php if ($totalRows_catesupdate > 0) { // Show if recordset not empty ?>
                    <table width="100%" border="0" cellspacing="0" cellpadding="0">
                      <tr>
                        <td><table width="100%" border="0" cellpadding="4" cellspacing="1" bgcolor="#ECCF5E">
                          <tr>
                            <td align="center" bgcolor="#FAF1D1" class="texts"><a href="categories.php" class="textsmallred">Back To Categories</a></td>
                          </tr>
                        </table>
                          <table width="40" border="0" cellpadding="0" cellspacing="0">
                            <tr>
                          ...
                          <table width="100%" border="0" cellpadding="4" cellspacing="1" bgcolor="#CCCCCC">
                          <tr>
                            <td background="images/leftNavBg.png" class="admintitle">category UPDATE</td>
                          </tr>
                        </table>
                          <table width="100%" border="0" cellpadding="8" cellspacing="1" bgcolor="#CCCCCC">
                            <tr>
                              <td height="47" align="left" bgcolor="#FFFFFF"><form action="<?php echo $editFormAction; ?>" method="POST" enctype="multipart/form-data" name="cateupdate" id="cateupdate">
                                <table width="100%" border="0" cellspacing="4" cellpadding="4">
                                  <tr>
                                    <td width="69" class="texts"><strong>CATENAME:</strong></td>
                                    <td colspan="2"><input name="catename" type="text" class="form" id="catename" value="<?php echo $row_catesupdate['catename']; ?>" style="text-transform:lowercase;" /></td>
                                  </tr>
                                  <tr>
                                    <td><input name="cateid" type="hidden" id="cateid" value="<?php echo $row_catesupdate['cateid']; ?>" /></td>
                                    <td width="82"><input name="button" type="submit" class="button" id="button" value="Update" /></td>
                                    <td width="491"><span class="textsmallred">( no space please )</span></td>
                                  </tr>
                                </table>
                                <input type="hidden" name="MM_update" value="cateupdate" />
                              </form>
                               <script language="JavaScript" type="text/javascript">

  var frmvalidator  = new Validator("cateupdate");
  frmvalidator.addValidation("catename","req","Please Enter Category");
  frmvalidator.addValidation("catename","maxlen=30","Max length for Category is 30");
  frmvalidator.addValidation("catename","alphanumeric","Only AlphaNumeric Characters Allow and No Space Please");
                    ...
                    <?php } // Show if recordset not empty ?>
                  <?php if ($totalRows_catesupdate == 0) { // Show if recordset empty ?>
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr>
      <td><table width="100%" border="0" cellpadding="4" cellspacing="1" bgcolor="#CCCCCC">
        <tr>
          <td background="images/leftNavBg.png" class="admintitle">ADD NEW CATEGORY</td>
          </tr>
        </table>
        <table width="100%" border="0" cellpadding="8" cellspacing="1" bgcolor="#CCCCCC">
          <tr>
            <td height="47" align="left" bgcolor="#FFFFFF"><form action="<?php echo $editFormAction; ?>" method="POST" enctype="multipart/form-data" name="cateinsert" id="cateinsert">
              <table width="100%" border="0" cellspacing="4" cellpadding="4">
                <tr>
                  <td width="69" class="texts"><strong>CATENAME:</strong></td>
                  <td colspan="2"><input name="catename" type="text" class="form" id="catename" style="text-transform:lowercase;" /></td>
                  </tr>
                <tr>
                  <td><input name="cateid" type="hidden" id="cateid" value="<?php echo $row_catesupdate['cateid']; ?>" />
                    <input name="selecttopic" type="hidden" id="selecttopic" value="<?php echo $row_setting['selecttopic']; ?>" /></td>
                  <td width="82"><input name="button2" type="submit" class="button" id="button2" value="Add" /></td>
                  <td width="491" class="textsmallred">( no space please )</td>
                  </tr>
                </table>
              <input type="hidden" name="MM_insert" value="cateinsert" />
              </form>
              <script language="JavaScript" type="text/javascript">

  var frmvalidator  = new Validator("cateinsert");
  frmvalidator.addValidation("catename","req","Please Enter Category");
  frmvalidator.addValidation("catename","maxlen=30","Max length for Category is 30");
  frmvalidator.addValidation("catename","alphanumeric","Only AlphaNumeric Characters Allow and No Space Please");
...
mysqli_free_result($catesupdate);
