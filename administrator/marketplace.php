...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "add")) {
   ...
   // UPLOAD CODE BY SYED RAZA ALI START //
	...
	$max_filesize = 5250000;
  	$upload_path = '../marketplace/images/market/';
  ...
  $insertSQL = sprintf("INSERT INTO market (title, `description`, imageurl, owner, `date`, price, catename, status, rating, views, `position`, users, editorpick, `current`) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
  ...
  $Result1 = mysqli_query(dbconnect(),$insertSQL) or die(mysqli_connect_error());
    ...
    $insertSQL = sprintf("INSERT INTO market (title, `date`, catename, users, selecttopic) VALUES (%s, %s, %s, %s, %s)",
                       GetSQLValueString($_POST['title'], "text"),
                       GetSQLValueString($_POST['date'], "text"),
                       GetSQLValueString($_POST['catename'], "text"),
                       GetSQLValueString($_POST['users'], "text"),
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "update")) {
  ...
  $updateSQL = sprintf("UPDATE market SET title=%s, `description`=%s, imageurl=%s, `date`=%s, price=%s, catename=%s, status=%s, rating=%s, views=%s, `position`=%s, users=%s, editorpick=%s, `current`=%s WHERE marketid=%s",
                       GetSQLValueString($_POST['title'], "text"),
                       GetSQLValueString($_POST['description'], "text"),
                       GetSQLValueString($filename, "text"),
                       GetSQLValueString($_POST['date'], "text"),
...
mysqli_select_db(dbconnect(),$database_rayicecms);
$query_blogoverview = "SELECT * FROM market ORDER BY marketid DESC";
...
if (isset($_GET['marketid'])) {
  $colname_blogupdate = $_GET['marketid'];
}
mysqli_select_db(dbconnect(),$database_rayicecms);
$query_blogupdate = sprintf("SELECT * FROM market WHERE marketid = %s", GetSQLValueString($colname_blogupdate, "int"));
...
mysqli_select_db(dbconnect(),$database_rayicecms);
$query_categorylist = "SELECT * FROM categories WHERE selecttopic = 'marketplace'";
...
<title><?php echo $row_setting['title']; ?> - MarketPlace</title>
...
<?php if($row_setting['selecttopic'] == 'custom')
			...
			if($row_contentlinks['status'] == 'active')
              ...
              <td width="700" height="30" align="center" valign="top"><?php if ($totalRows_blogupdate == 0) { // Show if recordset empty ?>
                  <table width="100%" border="0" cellpadding="4" cellspacing="1" bgcolor="#CCCCCC">
                    <tr>
                      <td background="images/leftNavBg.png" class="admintitle">MarketPlace OVERVIEW</td>
                    </tr>
                  </table>
  ...
  <table width="100%" border="0" cellspacing="2" cellpadding="4">
    ...
    <?php do { ?>
      <tr>
        <td height="11" align="center" class="smallbut"><div class="textsmallred"><?php echo $row_blogoverview['marketid']; ?></div></td>
        <td bgcolor="#FBFEFF" class="borderorangetable"><div class="texts"><?php echo $row_blogoverview['title']; ?></div></td>
        <td width="1" align="center" bgcolor="#FBFEFF" class="borderorangetable"><a href="marketplace.php?marketid=<?php echo $row_blogoverview['marketid']; ?>" class="textsmallgreen">
          <div>Edit</div>
        </a></td>
        <td width="1" align="center" bgcolor="#FBFEFF" class="borderorangetable"><a href="marketplace.php?delete=<?php echo $row_blogoverview['marketid']; ?>" class="textsmallred" onClick="javascript:return confirm('Please Confirm Before Delete ?')">
          <div>Delete</div>
        </a></td>
      </tr>
  ...
  <?php } // Show if recordset not empty ?>
                  ...
                  <?php if ($totalRows_blogupdate == 0) { // Show if recordset empty ?>
                    <table width="100%" border="0" cellpadding="4" cellspacing="1" bgcolor="#CCCCCC">
                      <tr>
                        <td background="images/leftNavBg.png" class="admintitle">ADD Product To MarketPlace</td>
                      </tr>
                    </table>
                    <table width="100%" border="0" cellpadding="8" cellspacing="1" bgcolor="#CCCCCC">
                      <tr>
                        <td height="47" align="left" bgcolor="#FFFFFF"><form action="<?php echo $editFormAction; ?>" method="POST" enctype="multipart/form-data" name="add" id="add">
                          <table width="100%" height="309" border="0" cellpadding="0" cellspacing="4">
                            <tr>
...
?>
                            ...
                            <tr>
                              <td height="24" class="texts"><strong>AUCTION:</strong></td>
                              <td><select name="current" class="formmenu" id="current">
                          ...
                          <input name="selecttopic" type="hidden" id="selecttopic" value="marketplace" />
...
<input type="hidden" name="MM_insert" value="add" />
  ...
  <table width="100%" border="0" cellpadding="4" cellspacing="1" bgcolor="#ECCF5E">
    <tr>
      <td align="center" bgcolor="#FAF1D1" class="texts"><a href="marketplace.php" class="textsmallred">Back To Market Place</a></td>
    </tr>
  </table>
  <table width="40" border="0" cellpadding="0" cellspacing="0">
    <tr>
  ...
  <table width="100%" border="0" cellpadding="4" cellspacing="1" bgcolor="#CCCCCC">
    <tr>
      <td background="images/leftNavBg.png" class="admintitle">UPDATE MARKETPLACE PRODUCT</td>
      </tr>
  </table>
                    <table width="100%" border="0" cellpadding="8" cellspacing="1" bgcolor="#CCCCCC">
                      <tr>
                        <td height="47" align="left" bgcolor="#FFFFFF"><form action="<?php echo $editFormAction; ?>" method="POST" enctype="multipart/form-data" name="update" id="update">
                          <table width="100%" height="309" border="0" cellpadding="0" cellspacing="4">
                            ...
                            <tr>
                              <td height="24" class="texts"><strong>DATE:</strong></td>
                              <td><input name="date" type="text" class="form" id="date" value="<?php echo $row_blogupdate['date']; ?>" /></td>
                              </tr>
                            <tr>
                              <td height="24" class="texts"><strong>CATEGORIES:</strong></td>
                              <td><select name="catename" class="formmenu" id="catename">
...
?>
                            ...
                            <tr>
                              <td height="24" class="texts"><strong>PRICE:</strong></td>
                              <td><input name="price" type="text" class="formmenusimple" id="price" value="<?php echo $row_blogupdate['price']; ?>" />
                                <?php echo $row_setting['currency']; ?></td>
                            </tr>
                            <tr>
                              <td height="24" class="texts"><strong>EDITIORS PICK:</strong></td>
                            ...
                            <tr>
                              <td height="24" class="texts"><strong>AUCTION:</strong></td>
                              <td><select name="current" class="formmenu" id="current">
                                <option value="open" <?php if (!(strcmp("open", $row_blogupdate['current']))) {echo "selected=\"selected\"";} ?>>open</option>
                                <option value="close" <?php if (!(strcmp("close", $row_blogupdate['current']))) {echo "selected=\"selected\"";} ?>>close</option>
                              </select></td>
                            </tr>
                            <tr>
                              <td height="24">&nbsp;</td>
                          ...
                          <input name="marketid" type="hidden" id="marketid" value="<?php echo $row_blogupdate['marketid']; ?>" />
